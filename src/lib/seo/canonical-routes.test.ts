...
describe("Day 1 & Day 2 SEO Technical Crawl & Canonical Verification", () => {
  it("generates exactly 59 unique canonical entries in /sitemap.xml", () => {
    ...
    // 1 Homepage + 5 Pillar Hubs + 1 Calculators Hub + 24 Calculators + 1 Guides Hub + 1 Research Hub + 11 Research Papers + 1 Datasets Hub + 8 Datasets + 7 Academic PDF Whitepapers + 3 OER Modules + 1 Standards + 7 Authority/Resource/Policy Pages = 72
    expect(urls.length).toBe(72);

    // Ensure zero duplicates
    const uniqueUrls = new Set(urls);
    expect(uniqueUrls.size).toBe(72);
